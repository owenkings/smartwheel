...
- `ultrasonic_adapter_node`：FD07-34 Modbus RTU 轮询，按配置发布 `/ultrasonic/range_*`
- `camera_adapter_node`：OpenCV/V4L2 采集，默认发布前摄像头图像

XT-M60 真实模式要求 `XTSDK_PY_ROOT` 或参数 `sdk_root` 指向官方 `xtsdk_py` 根目录。AGX Orin 上应使用 `lib/linux/aarch64` 下与 Python 版本匹配的 `.so`。

真实模式常用配置在 `wheelchair_bringup/config/h30_imu.yaml`、`ultrasonic.yaml`、`camera.yaml`。H30 默认 `/dev/smartwheel_h30_imu` + `460800`，超声波默认 `/dev/smartwheel_ultrasonic` + `9600`，摄像头默认设备索引 `0`。
