...
| Ericsii/livox_ros_driver2 | `378d1c7c9d33ec44d0683aef885b6ed0cce9612c` | MIT | None in mapping v2 | Required by the selected FAST-LIO ROS 2 fork; not evidence of XT-M60 compatibility |
...
Pins were recorded on 2026-07-14. `third_party/dependencies.repos` contains build
dependencies, including the nested `ikd-Tree` checkout. `third_party/references.repos`
contains source-comparison references that must not be placed in the ROS source tree.
ROS distribution packages remain managed through apt/rosdep and are pinned by the
versions above. No third-party mathematical core is copied into a `smartwheel_*`
package.
