...
两台默认都往主机同一 UDP 端口 7687 发,会互相串流 → `udp error`/frame discard。给两台设**不同 UDP 目的端口**(代码已支持 `setUdpDestIp`):
- `xtm60_left.yaml`:`udp_dest_ip: "192.168.0.100"`,`udp_dest_port: 7687`
- `xtm60_right.yaml`:`udp_dest_ip: "192.168.1.100"`,`udp_dest_port: 7688`
(默认 `udp_dest_port: 0` 关闭;先单台跑通再开双台。)

## 硬件/网络检查单(官方文档要点)
- **供电**:M60 需 12V/3A 或 19V/3.42A,电压不足直接影响点云质量。
- **网线/交换机**:两台雷达直连同一千兆交换机,**不要经 WiFi 路由器**;换优质网线。
- **防火墙**:确保不拦截 UDP(Linux 一般无 ufw;若开了 `sudo ufw allow 7687:7688/udp`)。
