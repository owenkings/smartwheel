...
## 2026-06-08 最终判定:右雷达确认损坏,系统改用单(左)雷达

复测右雷达 `192.168.1.101`:能 ping 通(网卡固件应答 ICMP),但 SDK TCP 控制
握手反复失败(`PortOpening-Disconnected` / `TxRxVerifying-Disconnected`),偶尔
握手成功时仍 `chip='0 0'`、停在 `Connected-Init`、不发点云。端口对齐与单独运行
均已排除软件因素。**判定为硬件故障,后续不再使用,等待固件修复或更换。**

### 单雷达可用性
当前代码**支持只用左雷达运行**:
- `dual_lidar_cloud_fusion_node` 的 `allow_single_lidar_fallback:true`(默认),
  任一路新鲜即继续发布 `/points_merged`,右路缺失只告警不崩溃。
- 静态建图、`/points_merged`、RTAB-Map 3D、`/scan` 投影、Nav2 规划、Web UI、
  IMU/超声波/相机/底盘全链路在单雷达下均可运行。
- **限制**:自主**运动**仍要求双雷达——使能电机时 fusion 以
  `allow_single_lidar_fallback:=false` 启动(见 `autonomous_rviz_mapping.launch.py`)。
  因此载人/自主行驶在右雷达修复前保持阻塞,这是预期的 fail-closed 行为。
