...
### 0.3 建图产品化路线：在线建图、录包建图和地图验收

必要性：高。当前 GUI 已能通过 `mapping.launch.py` 启动 `slam_toolbox` 在线建图，适合早期联调和小场景验证；但产品化建图需要更清晰的状态机、可复盘数据和地图质量验收，不能只依赖一次在线保存。

当前状态：
- 原生 GUI 的“开始建图”会自动从导航模式切换到建图模式，启动真实传感器、底盘里程计、点云转 `/scan` 和 `slam_toolbox`。
- 当前是在线建图流程：用户推行轮椅时地图同步生成；“结束并保存”只是保存当前 `/map`，不是离线批处理生成地图。
- GUI 建图页下方显示的是设备/话题检查，不是运行日志；建图日志落盘到 `.ros/log/mapping_*.log`。
- 没有可靠 `/wheel/odom` 时，真实建图只能作为传感器演示或半闭环测试，不能作为导航地图验收结果。

目标流程：
1. 首次建图前执行设备检查：`/scan`、`/wheel/odom`、TF、底盘状态、IMU 可选、超声波/摄像头可选。
2. 在线建图模式用于快速验证和小场景测试：用户低速推行轮椅，尽量闭环回到起点附近。
3. 增加 rosbag 录制模式：记录 `/scan`、`/xtm60/points`、`/wheel/odom`、`/imu/data`、`/tf`、`/tf_static`、`/base/status`、`/hardware/status`。
4. 增加离线建图模式：从 rosbag 重放后生成地图，允许重复调参、对比不同 `slam_toolbox` 参数和雷达投影参数。
5. 保存地图后自动执行质量检查：地图尺寸、未知区域比例、墙体连续性、闭环一致性、定位可用性。
6. 通过验收后生成显示层：运行 `vectorize_occupancy_map.py`，写入/更新 `semantic_map.yaml` 的墙线、房间、禁区和目标点图层。
7. 地图版本管理：保存 `maps/versions/<name>_<timestamp>.yaml`、当前别名 `maps/<name>.yaml`、质量报告、建图日志、录包文件路径和创建时间。

代码落点：
- `wheelchair_ui.mapping_manager`：扩展状态机为 `CHECKING -> RECORDING/ONLINE_MAPPING -> SAVING -> QUALITY_CHECK -> MAP_READY`，并维护 `maps/map_versions.json`。
- `wheelchair_mapping`：增加 `record_mapping_bag.py`、`offline_build_map.py`、`map_quality_check.py`。
- `wheelchair_bringup/launch/mapping.launch.py`：保留在线建图入口，补充录包和离线回放 launch。
- GUI 系统页：显示建图日志、rosbag 路径、质量报告路径和最近地图版本。
- 已实现：在线保存后的版本化落盘、当前地图别名、版本清单、质量报告关联和 GUI 最近版本切换。

验收标准：
- 固定安装雷达和底盘后，轮椅推行一圈可生成无明显重影的地图。
- 地图保存后可被 `full_system.launch.py map:=...` 加载，AMCL 能稳定定位。
- 在至少 3 次重复建图中，主要墙体和通道位置偏差在可接受范围内。
- 建图失败时 GUI 能给出明确原因，例如“无轮速里程计”“雷达数据中断”“闭环不足”“地图未知区域过多”。
...
- 使用策略：
  - 首次建图只生成基准静态地图。
  - 后续每次运行可持续记录传感器数据、定位轨迹和局部障碍，但只进入候选层。
  - 候选变化必须经过多次运行重复观测，并通过地图质量检查后，才能提示用户确认。
  - 用户确认后生成新地图版本；旧地图保留，可回滚。
  - 未确认候选不得参与全局导航，只能作为 UI 提示或局部代价地图的实时障碍。

优先级判断：P1。该功能很有产品价值，但必须排在首次建图、地图质量检查、地图版本管理和定位稳定性之后；在这些基础没有完成前，持续学习容易把定位漂移或临时障碍写入静态地图。
